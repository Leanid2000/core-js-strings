...
function removeFirstOccurrences(str, value) {
  ...
  return str.includes(value)
    ? str.slice(0, a) + str.slice(a + value.length)
    : str;
}
...
function formatTime(minutes, seconds) {
  const aa = minutes.toString();
  const bb = seconds.toString();
  const a = aa.padStart(2, '0');
  const b = bb.padStart(2, '0');
  return `${a}:${b}`;
}
...
function countVowels(str) {
  ...
  let summ = 0;
  str.split('').map((elem) => {
    if (arr.includes(elem.toLowerCase())) {
      summ += 1;
      return summ;
    }
    return summ;
...
function encodeToRot13(str) {
  ...
  const el = str.split('').map((elem) => {
    if (a.includes(elem)) {
      const num = a.indexOf(elem);
      return b[num];
    }
    return elem;
  });
